...
// Assembles a copy-pasteable prompt for an external AI chatbox, mirroring the existing
// masterPrompt/promptText/aiPrompt* prompt assembly used elsewhere in this modal for
// Ticker/BAR/other tiers — same source fields, just gathered into the "Arahan AI" tab.
function buildAiPrompt(fc: any, ceiling: { maxTitle: number; maxBrief: number; maxBriefLong: number }, hadTopik: number) {
  const lines = [
    ...
    `[Sumber dicadangkan]: ${fc.aiPromptSource || '-'}`,
    ...
    'Tajuk:', 'Topik:', 'Huraian ringkas:', 'Huraian panjang:', 'Sumber:', 'URL:', 'Tarikh:',
  ];
  return lines.join('\n');
}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-baseline justify-between gap-3">
        <span className={labelCls}>Bajet kandungan</span>
...
export const SlotManagerModal: React.FC<SlotManagerModalProps> = ({
  ...
  const items = useMemo(() => parseManualSummaryBlocks(formConfig.manualSummary || ''), [formConfig.manualSummary]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-md">
      <form onSubmit={onSave} className="bg-white rounded-lg border border-stone-200 shadow-2xl w-full max-w-[1080px] h-[min(88vh,720px)] max-h-full flex flex-col overflow-hidden animate-fade-in">
          ...
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-1.5 flex items-center gap-2.5 flex-wrap">
                ...
                <span className="text-stone-300">·</span>
                <span className="font-sans text-[11px] text-stone-500">Mod {formConfig.contentMode || 'Manual'}</span>
              </p>
            </div>
            ...
            <ol className="flex-1 min-h-0 overflow-y-auto list-none m-0 p-0 border-t border-stone-150">
              {items.map((it, i) => {
                ...
                return (
                  ...
                  >
                    <span className={`font-mono text-[11px] font-bold tabular-nums ${isActive ? 'text-[#802334]' : 'text-stone-400'}`}>{String(i + 1).padStart(2, '0')}</span>
                    <span className={`font-serif text-[13px] leading-snug truncate ${isActive ? 'text-stone-900 font-medium' : 'text-stone-600'}`}>
                      {it.title || <span className="text-stone-400 italic">Tiada tajuk</span>}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <span className="hidden group-hover:flex items-center gap-1.5">
                        <button type="button" aria-label="Naik" onClick={(e) => { e.stopPropagation(); move(i, -1); }} className="text-stone-500 hover:text-[#802334] px-0.5"><ChevronUp size={13} /></button>
          ...
          <section className="min-h-0 overflow-y-auto px-6 md:px-8 py-5 flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="flex flex-col gap-1 col-span-2">
                  <span className={`${labelCls} flex items-center gap-1.5`}>
                    Bidang {currentEditoriumRole !== 'KETUA_EDITOR' && <Lock className="w-3 h-3" />}
                  </span>
                  {currentEditoriumRole === 'KETUA_EDITOR' ? (
                    ...
                    <span className="font-serif text-sm text-stone-500 border-b border-stone-150 py-1.5">{desk || '— Belum ditetapkan —'}</span>
                  )}
                  <span className="font-sans text-[10px] text-stone-400 mt-0.5">Menukar Bidang akan mengarkibkan kandungan live sedia ada dalam slot ini.</span>
                </div>
            ...
            {tab === 'borang' && (
              <>
                ...
                <Field label="Huraian panjang" rows={5} value={current.briefLong || ''} placeholder="Huraian panjang, untuk paparan menatal penuh…" onChange={(v) => patch(activeIndex, 'briefLong', v)} />
            ...
            {tab === 'ai' && (
              <>
                <div>
                  <span className={labelCls}>Had aksara (untuk penjanaan AI)</span>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                    <ReadOnlyField label="Tajuk" value={String(ceiling.maxTitle)} />
                    <ReadOnlyField label="Huraian pendek" value={String(ceiling.maxBrief)} />
                    <ReadOnlyField label="Huraian panjang" value={String(ceiling.maxBriefLong)} />
                    <ReadOnlyField label="Topik" value={String(hadTopik)} />
                  </div>
                </div>

                <div className="flex flex-col gap-1">
                  <span className={labelCls}>Peraturan am (Sistem/Global) <span className="font-sans normal-case tracking-normal text-stone-400">· auto</span></span>
                ...
                <div className="grid grid-cols-2 gap-5">
                  ...
                  <Field label="Negara/Wilayah sumber" value={formConfig.aiPromptRegion || ''} onChange={(v) => setFormConfig((prev: any) => ({ ...prev, aiPromptRegion: v }))} />
                  <label className="flex flex-col gap-1">
                ...
                <div className="flex items-center justify-between gap-4">
                  <span className="font-sans text-[11px] text-stone-500">Salin arahan ini untuk ditampal ke chatbox AI. Output AI ditampal semula di "Borang kandungan".</span>
