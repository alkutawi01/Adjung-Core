...
// Sumber = Adjung sendiri (2026-08-16, permintaan Izzat — kad papar DUA tarikh nampak redundan:
// lencana "tarikh siaran" di bucu kad DAN baris tarikh sumber di bawah nama sumber, walhal
// kedua-duanya jatuh balik ke publishedAt YANG SAMA bila sumber ialah Adjung sendiri (bukan
// rujukan luar). Beza dua tarikh tu wujud utk bezakan "bila sumber ASAL diterbitkan" drpd "bila
// Adjung sendiri terbitkan" — pembezaan tu tak bermakna langsung bila sumbernya Adjung sendiri.
export const sumberAdjungSendiri = (source?: string): boolean => {
...
//
// SYARAT YANG MESTI DIPEGANG (2026-08-16, pepijat Izzat: "kad slot syariah mengembang tiba2
// kerana ada kandungan yg lebih besar dlm carousel") — kunci tinggi ni SAH HANYA jika LEBAR lajur
// kandungan kekal SAMA sepanjang putaran. `minHeight` ialah LANTAI, bukan siling: ia boleh halang
// kad mengecut, TAK PERNAH boleh halang kad membesar.
//
// Yang pecahkan syarat tu dahulu: lajur sumber (<a> tepi kanan kad HERO/STANDARD) hidup di LUAR
// carousel dan membaca `bentoNewsItems[n]` — iaitu item AKTIF yang sudah digabung (lihat
// resolvedItem, ~baris 2062), jadi nama sumbernya BERUBAH setiap putaran. Dengan `flex-shrink-0`
// + basis auto, lebar lajur tu = lebar teks sumber aktif. Diukur LANGSUNG pada laman sebenar
// (2026-08-16): slot 2 = 245px ("JURNAL SYARIAH DAN PENGAJIAN ISLAM") berbanding 99px ("ADJUNG
// BRIEF") pada slot STANDARD lain yang seni binanya SERUPA. Putaran mencuri/memulangkan ~146px
// daripada lajur kandungan -> SEMUA item bertindan membalut semula jadi lebih tinggi -> trek grid
// membesar -> kad mengembang, dan `maxSeen` kemudian mengunci nilai besar tu selamanya.
//
// Sebab tu lajur sumber sisi kini berlebar TETAP (`md:w-28` STANDARD, `md:w-36` HERO) — bukan
// hiasan, ia SYARAT KETEPATAN bagi kunci tinggi ni. JANGAN pulangkan ia kepada lebar-ikut-teks.
// Kunci tinggi FOOTER SUMBER (2026-08-17, Izzat: kad carousel tak stabil di mobile — "ia akan
// mengembang kalau ada tarikh, dan menguncup kalau takde tarikh... frontpage nampak bergerak2").
// Diukur langsung pada laman sebenar (mobile 375px): satu slot berputar 282.75px -> 271.375px
// tanpa sebarang interaksi pengguna, walhal kawasan tajuk+huraian yang dikunci CarouselStableBlock
// TERBUKTI tekal (96.25px kedua-dua item). Punca: footer sumber (nama + tarikh) ialah SIBLING di
// LUAR kawasan terkunci CarouselStableBlock, baca `bentoNewsItems[n]` (item aktif carousel yang
// berubah setiap putaran) — nama sumber panjang boleh balut lebih baris utk SATU item drpd item
// lain dlm putaran SAMA, jadi tinggi footer (dan seterusnya keseluruhan kad) berubah-ubah.
//
// SENGAJA komponen BERASINGAN drpd CarouselStableBlock (bukan pindah footer masuk renderItem) —
// struktur JSX renderItem tu sangat fragile (lihat nota `alignSelf:'start'` di atas, wujud khusus
// elak grid-stretch rosakkan ukuran scrollHeight). Guna teknik SAMA (render tersembunyi setiap
// varian sebenar, ukur scrollHeight, kunci ke MAX yang pernah dilihat — "hanya membesar tak pernah
// mengecut") tapi zero risiko kepada carousel utama. Bekas tersembunyi diletak position:absolute
// DALAM bekas induk (BentoInner, yang sudah `relative`) supaya ia ukur pada LEBAR SEBENAR (wrap
// teks bergantung lebar) — bukan lebar andaian.
const FooterHeightLock: React.FC<{
  items: any[];
  renderFooter: (item: any) => React.ReactNode;
  children: (minHeight: number | undefined) => React.ReactNode;
}> = ({ items, renderFooter, children }) => {
  const [minHeight, setMinHeight] = useState<number | undefined>(undefined);
  const refs = useRef<(HTMLDivElement | null)[]>([]);
  const contentKey = items
    .map((it) => `${it?.source || ''}|${it?.originalDate || ''}|${it?.publishedAt || ''}|${it?.sembunyikanTarikhSumber ? 1 : 0}`)
    .join('~');
  ...
  if (items.length <= 1) return <>{children(undefined)}</>;
...
// Pautan pantas "Edit kandungan ini" (2026-08-17, permintaan Izzat) — ikon pensel di bucu
// bawah-kanan setiap kad, HANYA kelihatan bila editor log masuk Editorium (currentEditoriumRole)
// DAN kandungan tu ada objectId (draf belum terbit TIADA objectId — sengaja sorok, bukan pautan
// rosak). Klik bawa terus ke Semakan Kandungan dengan itemId di URL; ContentReview.tsx baca
...
export const FrontpageView: React.FC<FrontpageViewProps> = ({
  ...
  const rawBentoNewsItems = React.useMemo(() => {
    ...
    for (let i = 0; i < 38; i++) {
      ...
      // Tiada baris "tarikh sumber" langsung bila sumber Adjung sendiri (2026-08-16, permintaan
      // Izzat — kad papar DUA tarikh nampak redundan sbb kedua-duanya jatuh balik ke publishedAt
      // YANG SAMA bila sumber bukan rujukan luar). `sembunyikanTarikhSumber` dibaca semua tapak
      // render (30 kad, ~baris 2900+) sbg pengawal SEBELUM fallback getDisplayDate||formatBentoDate
      // sedia ada — bukan gantikan logik fallback, cuma sekat ia terpakai utk kes ni.
      itemToPush.sembunyikanTarikhSumber = sumberAdjungSendiri(itemToPush.source);
      if (Array.isArray(itemToPush.items)) {
        itemToPush.items = itemToPush.items.map((it: any) => {
          const disatukan = Array.isArray(it.sources) && it.sources.length > 1
            ? { ...it, source: 'Editorial Adjung', originalDate: '' }
            : it;
          return { ...disatukan, sembunyikanTarikhSumber: sumberAdjungSendiri(disatukan.source) };
        });
      }
               ...
               style={getCardTheme(bentoNewsItems[0], 'transparent').cardStyle} >
                <BentoInner itemKey="0" className="md:flex-row md:items-center justify-between gap-6" aiProvider={bentoNewsItems[0].aiProvider}>
                  <div className="space-y-2 max-w-3xl">
                    {/* Kunci tinggi baris eyebrow (2026-08-28, pembetulan jurang eyebrow->tajuk
                        tak konsisten, laporan Izzat) — corak SAMA seperti SEGI_EMPAT_SMALL/KOMPAK
                        (lihat slot 3/4/5), dipanjangkan ke HERO/MENEGAK/STANDARD supaya jurang
                        eyebrow->tajuk kekal SAMA tak kira topik semasa pendek/panjang atau ada/
                        tiada ikon Bidang — dikunci ke tinggi topik TERPANJANG dalam putaran
                        carousel slot ni (bukan tinggi item aktif semasa sahaja). */}
                    <FooterHeightLock
                      items={bentoNewsItems[0].items && bentoNewsItems[0].items.length > 0 ? bentoNewsItems[0].items : [bentoNewsItems[0]]}
                      renderFooter={(it) => (
                        <div className="font-mono text-[9px] md:text-[10px] uppercase tracking-widest text-[#E9D8A6] font-bold"><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                      )}
                    >
                      {(minHeight) => (
                        <div className="font-mono text-[9px] md:text-[10px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[0]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[0]} bidang={bidangUntuk(bentoNewsItems[0])} onCari={cariDariEyebrow} />}</div>
                      )}
                    </FooterHeightLock>
                    <CarouselStableBlock
                      items={bentoNewsItems[0].items && bentoNewsItems[0].items.length > 0 ? bentoNewsItems[0].items : [bentoNewsItems[0]]}
                  ...
                  <BentoInner itemKey="1" className="gap-3" aiProvider={bentoNewsItems[1].aiProvider}>
                    <div className="space-y-4">
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#FFE3D1] font-bold mb-2"><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#FFE3D1] font-bold mb-2 flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[1]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[1]} bidang={bidangUntuk(bentoNewsItems[1])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[1].items && bentoNewsItems[1].items.length > 0 ? bentoNewsItems[1].items : [bentoNewsItems[1]]}
                  ...
                  <BentoInner itemKey="2" className="md:flex-row md:items-center justify-between gap-4" aiProvider={bentoNewsItems[2].aiProvider}>
                    <div className="flex-1">
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold"><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[2]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[2]} bidang={bidangUntuk(bentoNewsItems[2])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[2].items && bentoNewsItems[2].items.length > 0 ? bentoNewsItems[2].items : [bentoNewsItems[2]]}
                  ...
                  <BentoInner itemKey="3" className="gap-3" aiProvider={bentoNewsItems[3].aiProvider}>
                    <div>
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#F5EBE6] font-bold mb-2 flex flex-col justify-start" style={getCardTheme(bentoNewsItems[3]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                    ...
                    <BentoInner itemKey="4" className="gap-3" aiProvider={bentoNewsItems[4].aiProvider}>
                      <div>
                        <FooterHeightLock
                          ...
                          renderFooter={(it) => (
                            <div className="font-mono text-[9px] md:text-[8px] uppercase tracking-widest font-bold mb-1"><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                          )}
                        >
                          {(minHeight) => (
                        ...
                        <CarouselStableBlock
                          ...
                          renderEyebrow={(it) => (
                            <div className="font-mono text-[9px] md:text-[8px] uppercase tracking-widest text-[#D6D3D1] font-bold mb-1 flex flex-col justify-start" style={getCardTheme(bentoNewsItems[4]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                          )}
                          renderFooter={(it) => (
                      ...
                      >
                        {(minHeight) => (
                          <a href={bentoNewsItems[4].url || '#'} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="font-sans text-[7px] md:text-[8px] tracking-editorial uppercase text-stone-400 pt-1.5 border-t border-white/10 flex flex-col gap-0.5 mt-auto" style={{ ...getCardTheme(bentoNewsItems[4]).sourceStyle, minHeight }}>
                            <span>{bentoNewsItems[4].source}</span>
                            {!bentoNewsItems[4].sembunyikanTarikhSumber && (getDisplayDate(bentoNewsItems[4].originalDate) || formatBentoDate(bentoNewsItems[4].publishedAt)) && <span className="opacity-60 normal-case font-mono text-[7px] md:text-[7px]">{(getDisplayDate(bentoNewsItems[4].originalDate) || formatBentoDate(bentoNewsItems[4].publishedAt))}</span>}
                          </a>
                        )}
                      </FooterHeightLock>
                    ...
                    <BentoInner itemKey="5" className="gap-3" aiProvider={bentoNewsItems[5].aiProvider}>
                      <div>
                        ...
                        <CarouselStableBlock
                          ...
                          renderEyebrow={(it) => (
                            <div className="font-mono text-[9px] md:text-[8px] uppercase tracking-widest text-[#D6D3D1] font-bold mb-1 flex flex-col justify-start" style={getCardTheme(bentoNewsItems[5]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                          )}
                          renderFooter={(it) => (
                            <div className="font-sans text-[7px] md:text-[8px] tracking-editorial uppercase text-stone-400 pt-1.5 border-t border-white/10 flex flex-col gap-0.5">
                              <span>{it.source}</span>
                      ...
                      >
                        {(minHeight) => (
                          <a href={bentoNewsItems[5].url || '#'} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="font-sans text-[7px] md:text-[8px] tracking-editorial uppercase text-stone-400 pt-1.5 border-t border-white/10 flex flex-col gap-0.5 mt-auto" style={{ ...getCardTheme(bentoNewsItems[5]).sourceStyle, minHeight }}>
                            <span>{bentoNewsItems[5].source}</span>
                            {!bentoNewsItems[5].sembunyikanTarikhSumber && (getDisplayDate(bentoNewsItems[5].originalDate) || formatBentoDate(bentoNewsItems[5].publishedAt)) && <span className="opacity-60 normal-case font-mono text-[7px] md:text-[7px]">{(getDisplayDate(bentoNewsItems[5].originalDate) || formatBentoDate(bentoNewsItems[5].publishedAt))}</span>}
                          </a>
                        )}
                      </FooterHeightLock>
                 ...
                 style={getCardTheme(bentoNewsItems[6], 'transparent').cardStyle} >
                  <BentoInner itemKey="6" className="md:flex-row md:items-center justify-between gap-4" aiProvider={bentoNewsItems[6].aiProvider}>
                    <div className="flex-1">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[6]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[6]} bidang={bidangUntuk(bentoNewsItems[6])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[6].items && bentoNewsItems[6].items.length > 0 ? bentoNewsItems[6].items : [bentoNewsItems[6]]}
                 ...
                 style={{ ...getCardTheme(bentoNewsItems[12], 'transparent').cardStyle, ...bar1SiblingLocks.idx12.lockStyle }} >
                  <BentoInner itemKey="12" className="gap-3" aiProvider={bentoNewsItems[12].aiProvider}>
                    <div className="space-y-4">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#FFE3D1] font-bold mb-2 flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[12]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[12]} bidang={bidangUntuk(bentoNewsItems[12])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[12].items && bentoNewsItems[12].items.length > 0 ? bentoNewsItems[12].items : [bentoNewsItems[12]]}
                  ...
                  <BentoInner itemKey="11" className="gap-3" aiProvider={bentoNewsItems[11].aiProvider}>
                    <div>
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#F5EBE6] font-bold mb-2 flex flex-col justify-start" style={getCardTheme(bentoNewsItems[11]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                  ...
                  <BentoInner itemKey="13" className="gap-3" aiProvider={bentoNewsItems[13].aiProvider}>
                    <div className="space-y-2">
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={getCardTheme(bentoNewsItems[13]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                      ...
                      <CarouselStableBlock
                        ...
                        renderEyebrow={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#F5EBE6] font-bold flex flex-col justify-start" style={getCardTheme(bentoNewsItems[13]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                        renderFooter={(it) => (
                  ...
                  <BentoInner itemKey="14" className="gap-3" aiProvider={bentoNewsItems[14].aiProvider}>
                    <div className="space-y-2">
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={getCardTheme(bentoNewsItems[14]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                      ...
                      <CarouselStableBlock
                        ...
                        renderEyebrow={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#F5EBE6] font-bold flex flex-col justify-start" style={getCardTheme(bentoNewsItems[14]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                        renderFooter={(it) => (
                          <div className="font-sans text-[7px] md:text-[9px] tracking-editorial uppercase text-stone-300 pt-1.5 border-t border-white/10 flex flex-col gap-0.5">
                            <span>{it.source}</span>
                 ...
                 style={getCardTheme(bentoNewsItems[15], 'transparent').cardStyle} >
                  <BentoInner itemKey="15" className="gap-3" aiProvider={bentoNewsItems[15].aiProvider}>
                    <div className="space-y-4">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#FFE3D1] font-bold mb-2 flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[15]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[15]} bidang={bidangUntuk(bentoNewsItems[15])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[15].items && bentoNewsItems[15].items.length > 0 ? bentoNewsItems[15].items : [bentoNewsItems[15]]}
                  ...
                  <BentoInner itemKey="16" className="gap-3" aiProvider={bentoNewsItems[16].aiProvider}>
                    <div>
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#F5EBE6] font-bold mb-2 flex flex-col justify-start" style={getCardTheme(bentoNewsItems[16]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                ...
                <div 
                  ...
                  className={`p-4 relative rounded-lg shadow-sm hover:scale-[1.01] hover:shadow-lg transition-all duration-200 cursor-pointer flex flex-col min-h-[120px] flex-[1_0_auto] group`} 
                   style={getCardTheme(bentoNewsItems[17], 'transparent').cardStyle} >
                    ...
                    >
                      {(minHeight) => (
                        <a href={bentoNewsItems[17].url || '#'} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="font-sans text-[7px] md:text-[8px] tracking-editorial uppercase text-stone-400 pt-1.5 border-t border-white/10 flex flex-col gap-0.5 mt-auto" style={{ ...getCardTheme(bentoNewsItems[17]).sourceStyle, minHeight }}>
                          <span>{bentoNewsItems[17].source}</span>
                          {!bentoNewsItems[17].sembunyikanTarikhSumber && (getDisplayDate(bentoNewsItems[17].originalDate) || formatBentoDate(bentoNewsItems[17].publishedAt)) && <span className="opacity-60 normal-case font-mono text-[7px] md:text-[7px]">{(getDisplayDate(bentoNewsItems[17].originalDate) || formatBentoDate(bentoNewsItems[17].publishedAt))}</span>}
                        </a>
                      )}
                    </FooterHeightLock>
                ...
                <div 
                  ...
                  className={`p-4 relative rounded-lg shadow-sm hover:scale-[1.01] hover:shadow-lg transition-all duration-200 cursor-pointer flex flex-col min-h-[120px] flex-[1_0_auto] group`} 
                   style={getCardTheme(bentoNewsItems[18], 'transparent').cardStyle} >
                    ...
                    >
                      {(minHeight) => (
                        <a href={bentoNewsItems[18].url || '#'} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="font-sans text-[7px] md:text-[8px] tracking-editorial uppercase text-stone-400 pt-1.5 border-t border-white/10 flex flex-col gap-0.5 mt-auto" style={{ ...getCardTheme(bentoNewsItems[18]).sourceStyle, minHeight }}>
                          <span>{bentoNewsItems[18].source}</span>
                          {!bentoNewsItems[18].sembunyikanTarikhSumber && (getDisplayDate(bentoNewsItems[18].originalDate) || formatBentoDate(bentoNewsItems[18].publishedAt)) && <span className="opacity-60 normal-case font-mono text-[7px] md:text-[7px]">{(getDisplayDate(bentoNewsItems[18].originalDate) || formatBentoDate(bentoNewsItems[18].publishedAt))}</span>}
                        </a>
                      )}
                    </FooterHeightLock>
                 ...
                 style={getCardTheme(bentoNewsItems[19], 'transparent').cardStyle} >
                  <BentoInner itemKey="19" className="md:flex-row md:items-center justify-between gap-4" aiProvider={bentoNewsItems[19].aiProvider}>
                    <div className="flex-1">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[19]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[19]} bidang={bidangUntuk(bentoNewsItems[19])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[19].items && bentoNewsItems[19].items.length > 0 ? bentoNewsItems[19].items : [bentoNewsItems[19]]}
                 ...
                 style={{ ...getCardTheme(bentoNewsItems[26], 'transparent').cardStyle, ...bar2SiblingLocks.idx26.lockStyle }} >
                  <BentoInner itemKey="26" className="gap-3" aiProvider={bentoNewsItems[26].aiProvider}>
                    <div className="space-y-4">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#FFE3D1] font-bold mb-2 flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[26]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[26]} bidang={bidangUntuk(bentoNewsItems[26])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[26].items && bentoNewsItems[26].items.length > 0 ? bentoNewsItems[26].items : [bentoNewsItems[26]]}
                 ...
                 style={getCardTheme(bentoNewsItems[20], 'transparent').cardStyle} >
                  <BentoInner itemKey="20" className="md:flex-row md:items-center justify-between gap-4" aiProvider={bentoNewsItems[20].aiProvider}>
                    <div className="flex-1">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[20]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[20]} bidang={bidangUntuk(bentoNewsItems[20])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[20].items && bentoNewsItems[20].items.length > 0 ? bentoNewsItems[20].items : [bentoNewsItems[20]]}
                  ...
                  <BentoInner itemKey="25" className="gap-3" aiProvider={bentoNewsItems[25].aiProvider}>
                    <div>
                      <FooterHeightLock
                        ...
                        renderFooter={(it) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#F5EBE6] font-bold mb-2 flex flex-col justify-start" style={getCardTheme(bentoNewsItems[25]).deskStyle}><EyebrowKad item={it} bidang={bidangUntuk(it)} onCari={cariDariEyebrow} /></div>
                        )}
                      >
                        {(minHeight) => (
                 ...
                 style={getCardTheme(bentoNewsItems[29], 'transparent').cardStyle} >
                  <BentoInner itemKey="29" className="gap-3" aiProvider={bentoNewsItems[29].aiProvider}>
                    <div className="space-y-4">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#FFE3D1] font-bold mb-2 flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[29]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[29]} bidang={bidangUntuk(bentoNewsItems[29])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[29].items && bentoNewsItems[29].items.length > 0 ? bentoNewsItems[29].items : [bentoNewsItems[29]]}
                ...
                <div 
                  ...
                  className={`p-4 relative rounded-lg shadow-sm hover:scale-[1.01] hover:shadow-lg transition-all duration-200 cursor-pointer flex flex-col min-h-[120px] flex-[1_0_auto] group`} 
                   style={getCardTheme(bentoNewsItems[31], 'transparent').cardStyle} >
                    ...
                    >
                      {(minHeight) => (
                        <a href={bentoNewsItems[31].url || '#'} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="font-sans text-[7px] md:text-[8px] tracking-editorial uppercase text-stone-400 pt-1.5 border-t border-white/10 flex flex-col gap-0.5 mt-auto" style={{ ...getCardTheme(bentoNewsItems[31]).sourceStyle, minHeight }}>
                          <span>{bentoNewsItems[31].source}</span>
                          {!bentoNewsItems[31].sembunyikanTarikhSumber && (getDisplayDate(bentoNewsItems[31].originalDate) || formatBentoDate(bentoNewsItems[31].publishedAt)) && <span className="opacity-60 normal-case font-mono text-[7px] md:text-[7px]">{(getDisplayDate(bentoNewsItems[31].originalDate) || formatBentoDate(bentoNewsItems[31].publishedAt))}</span>}
                        </a>
                      )}
                    </FooterHeightLock>
                ...
                <div 
                  ...
                  className={`p-4 relative rounded-lg shadow-sm hover:scale-[1.01] hover:shadow-lg transition-all duration-200 cursor-pointer flex flex-col min-h-[120px] flex-[1_0_auto] group`} 
                   style={getCardTheme(bentoNewsItems[32], 'transparent').cardStyle} >
                    ...
                    >
                      {(minHeight) => (
                        <a href={bentoNewsItems[32].url || '#'} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="font-sans text-[7px] md:text-[8px] tracking-editorial uppercase text-stone-400 pt-1.5 border-t border-white/10 flex flex-col gap-0.5 mt-auto" style={{ ...getCardTheme(bentoNewsItems[32]).sourceStyle, minHeight }}>
                          <span>{bentoNewsItems[32].source}</span>
                          {!bentoNewsItems[32].sembunyikanTarikhSumber && (getDisplayDate(bentoNewsItems[32].originalDate) || formatBentoDate(bentoNewsItems[32].publishedAt)) && <span className="opacity-60 normal-case font-mono text-[7px] md:text-[7px]">{(getDisplayDate(bentoNewsItems[32].originalDate) || formatBentoDate(bentoNewsItems[32].publishedAt))}</span>}
                        </a>
                      )}
                    </FooterHeightLock>
                 ...
                 style={getCardTheme(bentoNewsItems[33], 'transparent').cardStyle} >
                  <BentoInner itemKey="33" className="md:flex-row md:items-center justify-between gap-4" aiProvider={bentoNewsItems[33].aiProvider}>
                    <div className="flex-1">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[33]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[33]} bidang={bidangUntuk(bentoNewsItems[33])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[33].items && bentoNewsItems[33].items.length > 0 ? bentoNewsItems[33].items : [bentoNewsItems[33]]}
                 ...
                 style={getCardTheme(bentoNewsItems[34], 'transparent').cardStyle} >
                  <BentoInner itemKey="34" className="md:flex-row md:items-center justify-between gap-4" aiProvider={bentoNewsItems[34].aiProvider}>
                    <div className="flex-1">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#E9D8A6] font-bold flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[34]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[34]} bidang={bidangUntuk(bentoNewsItems[34])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[34].items && bentoNewsItems[34].items.length > 0 ? bentoNewsItems[34].items : [bentoNewsItems[34]]}
                 ...
                 style={getCardTheme(bentoNewsItems[37], 'transparent').cardStyle} >
                  <BentoInner itemKey="37" className="gap-3" aiProvider={bentoNewsItems[37].aiProvider}>
                    <div className="space-y-4">
                      ...
                      >
                        {(minHeight) => (
                          <div className="font-mono text-[9px] uppercase tracking-widest text-[#FFE3D1] font-bold mb-2 flex flex-col justify-start" style={{ ...getCardTheme(bentoNewsItems[37]).deskStyle, minHeight }}>{<EyebrowKad item={bentoNewsItems[37]} bidang={bidangUntuk(bentoNewsItems[37])} onCari={cariDariEyebrow} />}</div>
                        )}
                      </FooterHeightLock>
                      <CarouselStableBlock
                        items={bentoNewsItems[37].items && bentoNewsItems[37].items.length > 0 ? bentoNewsItems[37].items : [bentoNewsItems[37]]}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-8 mb-8 px-4">
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <ul className="flex flex-col gap-1.5 font-sans text-xs text-stone-600 font-semibold flex-start">
                {halamanAktif('about') && <li className="flex"><button onClick={() => handleFooterLinkClick('about')} className="hover:text-[#802334] transition-colors text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-Adjung-maroon rounded-sm cursor-pointer">Mengenai Adjung</button></li>}
                {halamanAktif('editorial-board') && <li className="flex"><button onClick={() => handleFooterLinkClick('editorial-board')} className="hover:text-[#802334] transition-colors text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-Adjung-maroon rounded-sm cursor-pointer">Lembaga Editorial</button></li>}
                <li className="flex"><button onClick={() => handleFooterLinkClick('editors-notes')} className="hover:text-[#802334] transition-colors text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-Adjung-maroon rounded-sm cursor-pointer">Catatan Ketua Editor</button></li>
                {halamanAktif('publishing-policies') && <li className="flex"><button onClick={() => handleFooterLinkClick('publishing-policies')} className="hover:text-[#802334] transition-colors text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-Adjung-maroon rounded-sm cursor-pointer">Dasar Penerbitan</button></li>}
                <li className="flex"><button onClick={() => handleFooterLinkClick('notices')} className="hover:text-[#802334] transition-colors text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-Adjung-maroon rounded-sm cursor-pointer">Pengumuman</button></li>
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <ul className="flex flex-col gap-1.5 font-sans text-xs text-stone-600 font-semibold flex-start">
                ...
                <li className="flex"><Link to="/sertai-pasukan-editorial" className="hover:text-[#802334] transition-colors text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-Adjung-maroon rounded-sm cursor-pointer">Sertai Pasukan Editorial</Link></li>
                {halamanAktif('hubungi') && <li className="flex"><Link to="/hubungi" className="hover:text-[#802334] transition-colors text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-Adjung-maroon rounded-sm cursor-pointer">Hubungi</Link></li>}
